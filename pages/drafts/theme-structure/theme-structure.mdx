...
In this post I'll talk about what I mean 
- What exactly is a theme? 
- What does it consist of? 

The key to theming is using semantically named variables (e.g. background-color: `--color-bg-brand`) rather than literal values (e.g. background-color: `#ff0000`) when styling UI components. That way, when you want to switch themes you don't have to go through the code to replace every instance of a style. Instead, you simply change the values assigned to these semantically named variables in one central place.
