...
In the last post I talked about when and why it makes sense to use theming. But what exactly is a theme made up of? How does it work? And what does it take to implement?
...
For each theme, you define values for the semantic variables.
...
So then when you switch themes, the semantic variables are assigned the values for that theme, and that causes the look and feel of the UI to change. And all this happens without having to go through the code to manually change individual styles.

## Implementing Theming

So an individual *theme* is a table of semantic variables and their corresponding values. But implementing *theming* requires more than that. 
...
4. Configure each theme by defining values for the semantic variables. (Like in the table shown above.)
...
Themes shouldn't change the layout, relative hierarchy, or behavior of elements. So any properties that change those things should live with the component itself, not in the theme. 
...
To get to a manageable number of properties, you need to factor out common properties that are applied to multiple elements. 

For example, the same text color is applied in many places, so you'd factor it out into a common property that you can name and create a variable for. 

But how do you factor out common properties? It seems like a daunting task.
...
- To `text` 
- To convey `brand` 
- And where it has to be the most prominet or `priamry` element in the view
...
You'll notice that the value of the text property isn't the same for every element. The text color of a button is different from the text color of a heading. So you'll need a way to distinguish between the two.
