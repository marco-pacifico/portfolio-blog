...
In the last post I talked about when and why it makes sense to use theming. But what exactly is a theme made up of? How does theming work? And what does it take to implement it?
...
So an individual theme is a set of semantic variables and their corresponding values. But ***theming*** is more than that. 

Theming can be broken down into five main parts or steps:
...
### Exclude styles that change layout or relative hierarchy of elements
Theming is about changing the look and feel of a UI. Things like layout, hierarchy, and behavior of individual components shouldn't change from theme to theme. 

Any styles that fundamentally change the layout, hierarchy, or behavior should live with the component itself, not in the theme. For example, the button sizes aren't controlled by the theme. They're controlled by the button component itself. Same with the relative hierarchy of elements. The heading is always going to be bigger than paragraph text. That's not something that should change from theme to theme.

Think of the components as the building blocks of the UI. The theme is the paint that you apply to the building blocks. The paint doesn't change the shape of the building blocks. It just changes the color.
