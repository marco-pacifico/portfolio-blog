...
When a theme is applied, the semantic variables are assigned the values for that theme, and that causes the look and feel of the UI to change. And all this happens without having to go through the code to manually change individual styles.
{/* ```html
...
5. Apply the theme to the UI, assigning the values to the semantic variables.
...
Imagine you're the person using the variables to style a UI component. You want to be able to look at the property being styled and intuitively know what the corresponding variable name is. 
...
For example, the backdrop behind modals might have a unique background color used nowhere else in the UI.

Chaining modifiers together to come up with a variable name, you'd get:
...
The values chosen here determine the look and feel of each theme. They should come from a design spec where all values have been chosen to acheive the desired appearance, and also because they work well together and serve specific purposes in the UI.
...
These predefined literal values are called *Constants* or *Design Tokens*, and they're also just variables. But unlike semantic variables, constants are named in a way that literally describes the raw value they hold. Their names don't directly imply where or when they're meant to be used.

For example, the raw value `#1E1E1E` might be named `gray-950` since it's the darkest gray. 

<table style={{marginTop: "var(--space-6)", position: "relative"}}>
    <div style={{
      position: "absolute",
      inset: "0",
...
<table style={{marginBottom: "var(--space-6)", position: "relative"}}>
    <div style={{
      position: "absolute",
      inset: "0",
      maskImage: "linear-gradient(to top, black -10%, transparent 100%)",
...
### Scales and ramps
This ensures that the values are serving specific purposes, like creating hierarchy and providing interactive feedback, while also being cohesive and accessible.
Scales are a set of values that are designed to work together. They're usually designed to create relative hierarchy between eleemnts (via size and contrast), harmony, and consistency, while ensuring accessibility.
They're chosen specifically to work well together, to create relative hierarchy between elements (via size and contrast), harmony, and consistency, while ensuring accessibility.

Constants, as I'll call them, are another name for design tokens. They're a set of literal values that come from a design system. They're usually designed to work together, to create relative hierarchy between elements (via size and contrast), harmony, and consistency, while ensuring accessibility.

so that you're not just making up values as you go along. A predefined scale of values that you can choose from, that are all designed to work together, and that were chosen because they serve a specific purpose, usually to do with how they create contrast (for hierarchy and accessibility), how they work together (for harmony and consistency), and the meaning they convey.

### Rules for programatically choosing values
Rules that connect semantic names to values by mapping them to a scale of values. (e.g. color-bg-brand: color-brand-500 that comes from a design system.

Can be done one at a time or programmatically based on a few key values. Eg. primary color, secondary color, etc.

### One source of truth
In a table or similar data structure, choose values for each semantic variable for each theme. This is the one source of truth for the values of the semantic variables.
To map semantic names to values, you need a data structure that can hold this information. 

Figma makes it easy to create tables of semantic names and values. But this is not the best place to store this information because it's easily accessible to developers. It's also pretty tedius and time consuming to do all this data entry in Figma. 

Another option is to create a JSON file or a JS file with an object that maps semantic names to values. This file can be used to generate the CSS variables as well as the variables in Figma, using scripts and Figma plugins. 
