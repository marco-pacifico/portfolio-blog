...
Variable names need to imply what they're used for, hence semantic. They also need to be consistent and predictable. 

Imagine you're the person using the variables to style a UI component. You want to be able to look at the property being styled and be able to intuitively know what the corresponding variable name is. 
...
Here the "why" from the previous step is broken down further to specify "how much," meaning the level of contrast or size of the style.


### Chain together what + where + why + how much

For example, you'll definitely have a variable for the most prominent text color. So chaining together what + where + why + how much, you'd get something like this:
...
For example, you might have a variable for an expressive typeface that's used for headings. So chaining together what + where + why + how much, you'd get something like this:

`--font-text-brand-primary`

But the `font-family`property can only be applied to text and nothing in the "how much" column applies either. So you can drop those parts of the name, leaving you with:
...
### Specialty names for specific elements
You can try to style as many elements as possible with the general names defined above. They can often cover the majority of cases. 

But sometimes certain UI elements might be unique enough that they need variable names with even more specificy. 

The best way to do this is to add another modifier to the chain, where the modifier is the name of the component or element. 

Again, you can start with the most general part of the name and work your way down to the most specific part. Then cross out any redundant parts.

For example, the backdrop behind modals might need a variable for its color. Chaining modifiers together, you'd get:
...
Or even just, since it's implied that the only color on a backdrop is the background color:
...
The reason you want to keep color in there is because you might have other properties that apply to the backdrop, like `--blur-backdrop` or `--opacity-backdrop`. It's best to keep the naming consistent, going from general to specific.

### Create as few specialty variables as possible
One last callout here is to only create specialty names when the more general names don't work. The fewer variable names the beter. 
