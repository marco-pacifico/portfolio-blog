...
- How are palettes and scales used to support these jobs?
- How do these jobs map to semantic variables?

## Colors Perform Many Jobs
Colors have emotional and functional jobs to do in an interface. Broadly, they communicate meaning and guide a person's attention in a way that makes it easier for them to do what they came to do. 

It's because of these jobs that color is the most complex part of a theme and makes up the majority of semantic variables. 

To get a sense of the specific jobs colors perform, it's useful to look at each part of the naming convention for semantic variables and see how color fits in.

Looking back at the naming convention for semantic variables, color is the only type of property that can pick up a modifier from every column. Color is also the only type of visual property that can be broken down into two components: hue and shade. Where hue is the color itself, and shade is the amount of lightness or darkness of that color.
