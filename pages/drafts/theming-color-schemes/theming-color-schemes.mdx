...
{/* Color is the deepest part of theming. If you look at Shopify's Polaris or Adobe's Spectrum or Figma's design system, and count their theme variables, most of the variables are for colors. 

[Graphic: Theme variables -- percentatge color, typography, other styles]

Color is obviously important. But why exactly does a theme require so many color variables? It's because colors are doing a lot of work in an interface, and there's essentially a variable for each job performed. */}


{/* Color is obviously important.It's a highly noticeable part of brand's visual identity. But that's not why themes have so many color variables.

It turns out that color does a lot of work in an interface.  */}

{/* But the fact that's color noticeable and important isn't why so many v so is typography. What is it about color that requires so many theme variables?  */}

{/* And to have a theme that supports color in a robust way, you have to know exactly what color is doing and where.*/}


{/* Does a lot of work in a lot of places  */}

{/* In this post I'll explore why color turns out to be the deepest part of theming (i.e. why the majority of theme variables are for colors) and how to define the colors in a theme.  */}

{/* Along the way I'll answer the following questions:
...
As with all visual properties in a theme, color is used to express a brand's visual identity. Unlike changing the border radius */}

{/* ## Many Jobs */}
{/* Colors serve both emotional and functional purposes in an interface.  */}
Broadly speaking, color has two main jobs in an interface: 
- Convey meaning 
- Guide attention 

At more granular levels of detail in the UI, each high level job breaks down into a variety of more specific jobs, and each maps to a semantic variable. 

<table style={{marginBlock: "var(--space-6)", maxWidth: "400px"}}>
    <tr style={{height: "var(--space-7)"}}>
        <th style={{textAlign: "left"}}>Convey Meaning</th>
        <td>Level 1</td>
    </tr>
    <span style={{fontSize: "30px", margin: "16px"}}>⇣</span>
    <tr style={{height: "var(--space-7)"}}>
    ...
    <span style={{fontSize: "30px", margin: "16px"}}>⇣</span>
    <tr style={{height: "var(--space-7)"}}>
        <th style={{textAlign: "left"}}>Primary Button Background</th>
        <td>Level 3</td>
    </tr>
    <span style={{fontSize: "30px", margin: "16px"}}>⇣</span>
    <tr style={{height: "var(--space-7)"}}>
        <th style={{textAlign: "left"}}>Semantic Variable `color-bg-brand`</th>
        <td>Level 4</td>
    </tr>
</table>
...
{/* From these two high level jobs, a variety of other jobs emerge at more granular levels of detail in the interface. And it's this variety of jobs that makes color the most complex part of a theme. */}

{/* ## Hue, Saturation, Lightness 
Hue, saturation, lightnes */}

## Hues Convey Meaning
Brand identiy. Semantic. Neutral elements. 

Palette of hues, where each hue has a role to play in how it communicates meaning in the interface.


## Contrast Guides Attention
Contrast, change lightness or darknes

## Every Variable Represents a Job
So it's useful to look at each part of the naming convention for semantic variables to see how color fits in.
