...
Below is a simplified version of the component that renders the grid pattern for this interactive grid.
...
Each pattern is a component that renders a `<rect>` element that's filled by a `<pattern>` element.
...
To achieve the fade out effet, the grid and the other visual effects are wrapped in a div where the `mask-image` property is set to a radial gradient. 
...
In this case the mask image is a radial gradient that starts as solid white at the top center of the grid and then gradually transitions to transparent, creating a conical, spotlight shape. 

### Glow
The glow effect is another radial gradient. Like with the mask image, the center of the gradient is at the top center of the grid, creating a conical, spotlight shape that appears to shine down on the grid. 
<div
    style={{
      position: "relative",
      height: 320,
...
An interesting difference here is that the div containing the gradient is being centered at the top center of the grid, rather than the gradient itself.
...
The div is positioned using a combination of `top: 0`, `left: 50%`, and `transform: translate(-50%, -50%)`. With this combination of properties, the div can be any width or height and it will always be positioned at the top center of the grid. 

One implication of doing it this way is that you can set the width and height of the div to control the size of the glow effect.
...
To draw the decorations, I map through a list of coordinates to render a `<path>` element for the squares or `<circle>` elements for the circles. 
...
What I mean by paramertizing is making certain properties of the grid editable so that they can be dynamically manipulated.

This makes it possible to create a variety of grid patterns and visual effects. It's fun to play with the values and see what you can create. Doing this also helped me understand the fundamentals of how these patterns and effects are created.
...
Created inputs to adjust the parameters of the grid. CSS variables to avoid prop drilling and to makes code easier to read and understand
...
If you toggle between light and dark mode, the colors of the grid and inputs change. That's because the grid background, grid fill and strokes, and the inputs are pointing to global color variables that have both light and dark definitions defined in the root element.
